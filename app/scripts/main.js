...
$('.tlt').textillate();
  ...
  sr.reveal('.foo', {rotate: { x: 30, y: 30, z: 30 } });
  sr.reveal('.bar');
...
class Particle {
  constructor(canvas, options) {
    ...
    this.radius = options.radius || 0.2 + Math.random() * 6
    this.color = options.color || '#ff6228'
  ...
  swapColor() {
    if (this.radius > 1) {
      if (this.x < this.w / 2) {
        this.color = 'rgba(255, 134, 47, 0.73)'
      } else {
        this.color = '#ff2600'
      }
    }
